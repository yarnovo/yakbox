...
**yakbox** 是一个专业级的 React 聊天组件库，专注于提供高性能、可定制的基础聊天界面解决方案。项目的核心理念是"专业、高效、纯净"，通过现代化的技术架构和精心设计的 API，让开发者能够快速构建出生产级别的聊天应用。项目坚持单一职责原则，专注于聊天界面的核心功能，不涉及内容渲染的复杂性。
...
yakbox 在 React 生态中的定位是提供专业、纯净的聊天界面组件。不同于包含内容渲染的复合组件，yakbox 专注于聊天交互逻辑这一核心领域，为开发者提供稳定可靠的基础设施。项目遵循 Unix 哲学中"做好一件事"的原则，与现代前端工具链深度集成，支持 SSR、微前端等高级架构模式。
...
4. **关注点分离**: 界面逻辑与内容渲染职责分离

### 内容渲染最佳实践

1. **边界清晰原则**
   - yakbox 专注聊天界面交互，不处理内容解析
   - 复杂内容渲染应通过包装组件或独立包实现
   - 保持组件库的纯净性和可维护性
...
**背景**: 项目初期尝试集成 Markdown 和 XML 组件渲染功能

**决策**: 移除所有内容渲染相关功能，回归纯文本显示

**原因**:

1. **职责边界不清**: 内容渲染与聊天界面属于不同的关注点
2. **复杂度膨胀**: Markdown 解析、插件系统增加了维护成本
3. **用户需求多样**: 不同场景对内容渲染的需求差异很大
4. **Unix 哲学**: 遵循"做好一件事"的设计原则

**实施内容**:

- 删除 `MarkdownRenderer`、`EnhancedMarkdownRenderer`、`ReasoningBlock` 组件
- 移除 react-markdown、remark 系列等依赖
- 简化 MessageBubble 为纯文本显示
- 更新文档和示例
...
<!-- 最后更新时间: 2025-07-09T15:48:00+08:00 -->
<!-- 更新说明: 重大架构决策 - 移除内容渲染功能，回归纯文本聊天组件的核心定位，更新项目愿景、设计哲学和演进历史 -->
